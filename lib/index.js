...
var getCurrentPage = async () => {
  const confluenceUrl = getInput4("CONFLUENCE_URL");
  const username = getInput4("CONFLUENCE_USER");
  const password = getInput4("CONFLUENCE_TOKEN");
  const contentId = getInput4("CONTENT_ID");
...
var getInput4 = (name) => {
  const input = core2.getInput(name) || process.env[name];
  if (!input) {
    throw new Error(`Required input ${name} not provided`);
  }
  return input;
...
var publishDependencies = async (dependencies, page) => {
  const confluenceUrl = core4.getInput("CONFLUENCE_URL");
  const username = core4.getInput("CONFLUENCE_USER");
  const password = core4.getInput("CONFLUENCE_TOKEN");
  const contentId = core4.getInput("CONTENT_ID");
