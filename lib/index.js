...
var getCurrentDependencies = async () => {
  const confluenceUrl = getInput2("confluence-url");
  const username = getInput2("confluence-user");
  const password = getInput2("confluence-token");
  const contentId = getInput2("confluence-content-id");
...
var addServiceName = (names, name) => names.includes(name) ? names : names.concat(name);
...
var parseDependencies = (json, name) => {
...
var publishDependencies = async (dependencies, page) => {
  const confluenceUrl = core3.getInput("confluence-url");
  const username = core3.getInput("confluence-user");
  const password = core3.getInput("confluence-token");
  const contentId = core3.getInput("confluence-content-id");
  const markup = createMarkup(dependencies);
  core3.debug(`Will publish to the "${page.title}" page`);
  const url2 = `${confluenceUrl}/wiki/rest/api/content/${contentId}`;
  await axios_default({
    method: "put",
    url: url2,
...
async function run() {
  try {
    const dependenciesJson = core4.getInput("dependencies");
    if (dependenciesJson === "") {
      core4.debug(`Aborting without error. Got no dependencies`);
      return;
    }
