...
// src/assertNonNullObject.ts
var assertNonNullObject = (value) => typeof value === "object" && value !== null;

// src/parse/assertAtlasDocFormatResponse.ts
var assertAtlasDocFormatResponse = (res) => {
...
// src/parse/assertConfluenceDocument.ts
var assertConfluenceDocument = (data) => typeof data === "object" && data !== null && "type" in data && data.type === "doc";

// src/parse/assertTableContent.ts
var assertTableContent = (content) => content.type === "table";

// src/parse/assertTableContentRow.ts
var assertTableContentRow = (row) => {
  var _a, _b;
  return ((_b = (_a = row.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.type) === "tableCell";
};

// src/parse/assertTableHeaderRow.ts
var assertTableHeaderRow = (row) => {
  var _a, _b;
  return ((_b = (_a = row.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.type) === "tableHeader";
};

// src/parse/getCellValue.ts
var getCellValue = (row, index) => {
  var _a;
  return ((_a = row.content[index].content[0].content) == null ? void 0 : _a[0].text) ?? "";
};

// src/parse/convertTableRowsToDependencies.ts
var convertTableRowsToDependencies = (rows, mapping) => rows.reduce((acc, row) => {
  acc.push({
...
// src/parse/getDependencyPropIndexMapping.ts
function getDependencyPropIndexMapping(row) {
  if (!row) {
    return {
      name: 0,
      version: 1,
...
var findPropIndex = (searchValue, checkStart = false) => (tableHeader) => tableHeader.content.some(
  (paragraph) => {
    var _a;
    return (_a = paragraph.content) == null ? void 0 : _a.some((text) => checkStart ? text.text.toLowerCase().startsWith(searchValue) : text.text.toLowerCase() === searchValue);
  }
);

// src/parse/parseTable.ts
function parseTable(value) {
  if (!assertConfluenceDocument(value)) {
    throw new Error("Not a valid Confluence document");
  }
  try {
    const table = value.content.find(assertTableContent);
    if (!table) {
      return [];
    }
...
// src/getCurrentDependencies.ts
var getCurrentDependencies = async () => {
  const confluenceUrl = getInput2("CONFLUENCE_URL");
  const username = getInput2("CONFLUENCE_USER");
  const password = getInput2("CONFLUENCE_TOKEN");
  const contentId = getInput2("CONTENT_ID");
  const url2 = `${confluenceUrl}/wiki/rest/api/content/${contentId}?expand=body.atlas_doc_format`;
  ...
  const data = JSON.parse(res.data.body.atlas_doc_format.value);
...
// src/getCurrentPage.ts
var core2 = __toESM(require_core());

// src/parse/assertPage.ts
var assertPage = (res) => {
  if (!assertNonNullObject(res.data)) {
    return false;
  }
...
var getCurrentPage = async () => {
  const confluenceUrl = getInput4("confluence-url");
  const username = getInput4("confluence-user");
  const password = getInput4("confluence-token");
  const contentId = getInput4("confluence-content-id");
  const url2 = `${confluenceUrl}/wiki/rest/api/content/${contentId}`;
  const res = await axios_default({
    method: "get",
    url: url2,
    auth: {
...
// src/getDependencyInputName.ts
var core3 = __toESM(require_core());
var getDependencyInputName = () => core3.getInput("dependency-input-name") || "external-dependencies";
...
// src/mutMergeDependencies.ts
var mutMergeDependencies = (wikiDependencies, serviceDependencies, name) => {
...
var splitServiceNames = (services) => (services == null ? void 0 : services.split(", ")) ?? [];
var sortNamesAsc = (serviceNames) => serviceNames.sort((a, b) => a.localeCompare(b));
var sortDependenciesAsc = (dependencies) => dependencies.sort((a, b) => a.name.localeCompare(b.name) || a.version.localeCompare(b.version));
var joinServiceNames = (serviceNames) => sortNamesAsc(serviceNames).join(", ");
var removeServiceName = (names, name) => names.filter((n) => n !== name);
var addServiceName = (names, name) => names.concat(name);

// src/parseDependencies.ts
var assertArray = (data) => Array.isArray(data);
var assertObject = (data) => typeof data === "object";
var assertDependency = (dep) => assertObject(dep) && "name" in dep && "version" in dep && "license" in dep;
var assertDependencies = (data) => data.every(assertDependency);
var parseDependencies = (json) => {
...
// src/publishDependencies.ts
var core4 = __toESM(require_core());

// src/createMarkup.ts
var createMarkup = (dependencies) => {
...
// src/publishDependencies.ts
var publishDependencies = async (dependencies, page) => {
  const confluenceUrl = core4.getInput("confluence-url");
  const username = core4.getInput("confluence-user");
  const password = core4.getInput("confluence-token");
  const contentId = core4.getInput("confluence-content-id");
  const markup = createMarkup(dependencies);
  core4.debug(`Will publish to the "${page.title}" page`);
  const url2 = `${confluenceUrl}/wiki/rest/api/content/${contentId}`;
  await axios_default({
    method: "put",
    url: url2,
...
async function run() {
  try {
    ...
    const dependenciesJson = core5.getInput(dependencyInputName);
    if (dependenciesJson === "") {
      core5.debug(`Aborting without error. Got no dependencies from the ${dependencyInputName} input`);
      return;
    }
